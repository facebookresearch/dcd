...
def parse_args():
    ...
    parser.add_argument("-t", "--num_test_episodes", type=int, default=10)
    parser.add_argument(
        "--savename",
        type=str,
        default="latest",
    ...
    parser.add_argument(
        "--figsize", type=str, default="2,2", help="Dimensions of output figure."
    )
...
if __name__ == "__main__":
        ...
        with open(fpath, mode="r", newline="") as csvfile:
            ...
            for row in csvreader:
                if row[0].startswith(args.row_prefix):
                    agg_stats = agg_test_episodes_by_seed(
                        row[1:], args.num_test_episodes
                    )
    ...
    legend = plt.legend(
        ncol=1, loc="upper left", bbox_to_anchor=(1, 1), frameon=False, fontsize=12
    )
