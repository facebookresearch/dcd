...
class CovarGMM:
    def __init__(self, mins, maxs, seed=None, params=dict()):
        ...
        self.absolute_lp = (
            False if "absolute_lp" not in params else params["absolute_lp"]
        )
    ...
    def update(self, task, reward):
        ...
        self.tasks_times_rewards.append(
            np.array(task.tolist() + [current_time] + [reward])
        )
        ...
        if len(self.tasks) >= self.nb_random:  # If initial bootstrapping is done
            if (len(self.tasks) % self.fit_rate) == 0:  # Time to fit
                ...
                cur_tasks_times_rewards = np.array(
                    self.tasks_times_rewards[-self.fit_rate :]
                )

                # 2 - Fit batch of GMMs with varying number of Gaussians
                potential_gmms = [
                    GMM(n_components=k, covariance_type="full")
                    for k in self.potential_ks
                ]
                potential_gmms = [
                    g.fit(cur_tasks_times_rewards) for g in potential_gmms
                ]
