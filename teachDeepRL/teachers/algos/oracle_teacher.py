...
class OracleTeacher:
    def __init__(
        self, mins, maxs, window_step_vector, seed=None, reward_thr=230, step_rate=50
    ):
        self.seed = seed
        if not seed:
        ...
        self.window_pos = np.zeros(
            len(self.mins), dtype=np.float32
        )  # stores bottom left point of window
        for i, step in enumerate(self.window_step_vector):
            if (
                step > 0
            ):  # if step is positive, we go from min to max (and thus start at min)
        ...
        print(
            "window range:{} \n position:{}\n step:{}\n".format(
                self.window_range, self.window_pos, self.window_step_vector
            )
        )
            ...
            if mean_reward > self.reward_thr:
                for i, step in enumerate(self.window_step_vector):
                    if step > 0:  # check if not stepping over max
                        self.window_pos[i] = min(
                            self.window_pos[i] + step,
                            self.maxs[i] - self.window_range[i],
                        )
                    elif step <= 0:  # check if not stepping below min
                        self.window_pos[i] = max(
                            self.window_pos[i] + step, self.mins[i]
                        )
                print(
                    "mut stump: mean_ret:{} window_pos:({})".format(
                        mean_reward, self.window_pos
                    )
                )

    def sample_task(self):
        task = np.random.uniform(
            self.window_pos, self.window_pos + self.window_range
        ).astype(np.float32)
        # print(task)
        return task
