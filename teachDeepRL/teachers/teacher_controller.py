...
def param_vec_to_param_dict(param_env_bounds, param):
    ...
    for i, (name, bounds) in enumerate(param_env_bounds.items()):
        if len(bounds) == 2:
            ...
            len(bounds) == 3
        ):  # third value is the number of dimensions having these bounds
            nb_dims = bounds[2]
...
def param_dict_to_param_vec(
    param_env_bounds, param_dict
):  # needs param_env_bounds for order reference
    param_vec = []
...
class TeacherController(object):
    ...
    def dump(self, filename):
        with open(filename, "wb") as handle:
            dump_dict = {
                "env_params_train": self.env_params_train,
