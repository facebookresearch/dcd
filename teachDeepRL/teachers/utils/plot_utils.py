...
def plot_regions(
    boxes, interests, ax=None, xlabel="", ylabel="", xlim=None, ylim=None, bar=True
):
    ft_off = 15
    ...
    for b, ints in zip(boxes, interests):
        ...
        rect = patches.Rectangle(
            [lx, ly], (hx - lx), (hy - ly), linewidth=3, edgecolor="white", facecolor=c
        )
        ax.add_patch(rect)
        ...
        if i > 0 and (i % plot_step == 0):
            ...
            for j in range(len(all_boxes)):
                if iterations[j] > i:
                    ...
                    cur_idx = j
            plot_regions(
                all_boxes[cur_idx], interests[cur_idx], ax=ax[0], xlim=xlim, ylim=ylim
            )
...
def draw_ellipse(position, covariance, ax=None, color=None, **kwargs):
    ...
    for nsig in range(2, 3):
        ax.add_patch(
            Ellipse(position, nsig * width, nsig * height, angle, **kwargs, color=color)
        )
...
def gmm_plot_gif(
    ...
    gs_lps = bk["tasks_lps"]
    for i, (ws, covs, means, ep) in enumerate(
        zip(bk["weights"], bk["covariances"], bk["means"], bk["episodes"])
    ):
        ...
        plot_gmm(
            ...
            means,
            covs,
            np.array(gs_lps[old_ep + gen_size : ep + gen_size]),
            ax=ax,
            xlim=xlim,
            ylim=ylim,
            bar=bar,
        )  # add gen_size to have gmm + the points that they generated, not they fitted
        if "comp_grids" in bk:  # add competence grid info
            draw_competence_grid(
                ax, bk["comp_grids"][i], bk["comp_xs"][i], bk["comp_ys"][i]
            )
        if "start_points" in bk:  # add lineworld info
            draw_lineworld_info(
                ax, bk["start_points"], bk["end_points"], bk["current_states"][i]
            )
        f_name = gifdir + tmpdir + gifname + "_{}.png".format(ep)
        if title:
            plt.suptitle(
                "Episode {} | nb Gaussians:{}".format(ep, len(means)), fontsize=20
            )
        old_ep = ep
        if save_imgs:
...
def random_plot_gif(
    ...
    for i, (c_grids, c_xs, c_ys) in enumerate(
        zip(bk["comp_grids"], bk["comp_xs"], bk["comp_ys"])
    ):
        plt.figure(figsize=fig_size)
        ax = plt.gca()
        draw_competence_grid(ax, c_grids, c_xs, c_ys)
        ax.scatter(
            tasks[i * step : (i + 1) * step, 0],
            tasks[i * step : (i + 1) * step, 1],
            c="blue",
            s=2,
            zorder=2,
        )
