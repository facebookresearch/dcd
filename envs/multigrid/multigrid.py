...
class MultiGridEnv(minigrid.MiniGridEnv):
    ...
    def __init__(
        ...
        if self.minigrid_mode:
            ...
            self.action_space = gym.spaces.Box(
                low=0, high=len(self.actions) - 1, shape=(self.n_agents,), dtype="int64"
            )
    ...
    def reset(self):
        ...
        for a in range(self.n_agents):
            ...
            assert (
                start_cell.type == "agent"
                or start_cell is None
                or start_cell.can_overlap()
            )
    ...
    def __str__(self):
        ...
        for j in range(self.grid.height):
            for i in range(self.grid.width):
                ...
                agent_here = False
                for a in range(self.n_agents):
                    if self.agent_pos[a] is not None and (
                        i == self.agent_pos[a][0] and j == self.agent_pos[a][1]
                    ):
    ...
    def move_agent(self, agent_id, new_pos):
        ...
        assert (
            self.grid.get(new_pos[0], new_pos[1]).cur_pos == self.agent_pos[agent_id]
        ).all()
    ...
    def render(
        self, mode="human", close=False, highlight=True, tile_size=minigrid.TILE_PIXELS
    ):
