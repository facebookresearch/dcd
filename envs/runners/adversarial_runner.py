...
from util import array_to_csv, is_discrete_actions, get_obs_at_index, set_obs_at_index
...
class AdversarialRunner(object):
    ...
    def _calculate_paired_regret_scores(
        ...
        if type == "paired":
            ...
            env_mean_return[adversary_agent_max_idx] = agent_rollout_info[
                "mean_return"
            ][adversary_agent_max_idx]
            env_mean_return[agent_max_idx] = adversary_agent_rollout_info[
                "mean_return"
            ][agent_max_idx]
    ...
    def _get_env_stats_multigrid(self, agent_info, adversary_agent_info):
        ...
        passable_ratio = np.mean(
            agent_info.get("passable_ratio", self.venv.get_passable())
        )
        ...
        if solved_idx is None:
            if "max_returns" in adversary_agent_info:
                solved_idx = (
                    (
                        torch.max(
                            agent_info["max_return"], adversary_agent_info["max_return"]
                        )
                        > 0
                    )
                    .numpy()
    ...
    def _get_env_stats(
        self, agent_info, adversary_agent_info, log_replay_complexity=False
    ):
        env_name = self.args.env_name
    ...
    def _update_plr_with_current_unseen_levels(self, parent_seeds=None):
        ...
        self.current_level_seeds = self.level_store.insert(
            levels, parent_seeds=parent_seeds
        )
    ...
    def agent_rollout(
        ...
        if is_env:
            ...
            if level_replay:  # Get replay levels
                ...
                levels = [
                    self.level_store.get_level(seed)
        ...
        for step in range(num_steps):
                    ...
                    if not is_env:
                        ...
                        if agent.storage.use_proper_time_limits:
                            if "truncated_obs" in info.keys():
                                ...
                                agent.storage.insert_truncated_obs(
                                    truncated_obs, index=i
                                )
            ...
            if (not is_env) and level_sampler:
                current_level_seeds = torch.tensor(
                    self.current_level_seeds, dtype=torch.int
                ).view(-1, 1)
    ...
    def run(self):
        ...
        no_exploratory_grad_updates = vars(args).get(
        ...
        if (
            level_replay
            and self.is_paired
            and (args.protagonist_plr == args.antagonist_plr)
        ):
            self.agent_rollout(
                agent=adversary_env,
                num_steps=self.adversary_env_rollout_steps,
        ...
        if self.is_paired:
            ...
            external_scores = self._calculate_paired_regret_scores(
                agent_info, adversary_agent_info, type=args.accel_paired_score_function
            )
            ...
            if self.is_training and self.args.use_behavioural_cloning:
                if (self.student_grad_updates) % self.args.kl_update_step == 0:
                    kl_dict_agent = {}
                    adversary_agent.eval()
                    kl_dict_agent["antagonist_model"] = (
                        adversary_agent.algo.actor_critic
                    )
        ...
        if level_replay:
            sampled_level_info = {
                "level_replay": True,
                "num_edits": [
                    len(self.level_store.seed2parent[x]) + 1 for x in env_info
                ],
            }
        else:
            sampled_level_info = {
        ...
        if level_replay and edit_level:
            ...
            if self.base_levels == "batch":
                ...
                if self.use_accel_paired:
                    ...
                    regret_score = self._calculate_paired_regret_scores(
                        agent_info,
                ...
                if args.num_processes >= 4:
                    ...
                    if self.use_accel_paired:
                        easy = list(
                            np.argsort((regret_score.detach().cpu().numpy()).flatten())[
                                :4
                            ]
                        )
                    else:
                        easy = list(
                            np.argsort(
                                (
                                    agent_info["mean_return"].detach().cpu().numpy()
                                    - agent_info["batched_value_loss"]
                                    .detach()
                                    .cpu()
                                    .numpy()
                                ).flatten()
                    ...
                    fixed_seeds = [env_info[x.item()] for x in easy] * int(
                    ...
                    if self.use_accel_paired:
                        easy = np.argmax(
                            (regret_score.detach().cpu().numpy()).flatten()
                        )
                    else:
                        easy = np.argmax(
                            (
        ...
        if self.is_training and self.is_training_env:
            ...
            env_value_loss, env_action_loss, env_dist_entropy, info = (
                adversary_env.update()
            )
        ...
        if (not level_replay) or log_replay_complexity:
            ...
            if args.use_plr:
                self.latest_env_stats.update(
                    stats
                )  # Log latest UED curriculum stats instead of PLR env stats
        else:
        ...
        if args.log_grad_norm:
            ...
            if self.is_paired:
                adversary_grad_norm = np.mean(
                    adversary_agent_info["update_info"]["grad_norms"]
                )
            if self.is_training_env:
                adversary_env_grad_norm = np.mean(
                    adversary_env_info["update_info"]["grad_norms"]
                )
            stats.update(
                {
            ...
            stats.update(
                {
                    ...
                    "adversary_action_complexity": adversary_agent_info[
                        "action_complexity"
                    ],
                }
            )
