...
class CarRacingBezierAdversarial(CarRacingBezier):
    def __init__(
        ...
        self.adversary_control_points_obs_space = gym.spaces.Box(
            low=0, high=sketch_dim, shape=(1, sketch_dim, sketch_dim), dtype="uint8"
        )
        ...
        if use_categorical:
            action_low, action_high = np.array((0,)), np.array(
                (self.sketch_dim**2 + 1,)
            )  # +1 for skip action
            if sparse_rewards:
    ...
    def _closest_track_index(self, alpha):
        ...
        u = np.mean(np.array(self._adversary_control_points), axis=0)
        track_alphas = np.array(
            [self._alpha_from_xy(x - u[0], y - u[1]) for _, _, x, y in self.track]
        )
    ...
    def unnormalize_xy(self, x, y):
        scaled_x = int(
            np.minimum(
                np.maximum(np.round(self.sketch_dim * x), 0), self.sketch_dim - 1
            )
        )
        scaled_y = int(
            np.minimum(
                np.maximum(np.round(self.sketch_dim * y), 0), self.sketch_dim - 1
            )
        )
    ...
    def step_adversary(self, action):
        ...
        obs = {
            "control_points": np.expand_dims(
                self._adversary_sketch, 0
            ),  # 1 x sketch_dim x sketch_dim
            "time_step": [self.adversary_step_count],
            "random_z": self.generate_random_z(),
        }
