...
def _flatten_helper(T, N, _tensor):
    if isinstance(_tensor, dict):
        return {
            k: _tensor[k].view(T * N, *_tensor[k].size()[2:]) for k in _tensor.keys()
        }
    else:
...
class RolloutStorage(object):
    def __init__(
        ...
        recurrent_hidden_state_buffer_size = (
            2 * recurrent_hidden_state_size
            if self.is_lstm
        ...
        if isinstance(observation_space, dict):
            ...
            self.obs = {
                k: torch.zeros(
                    num_steps + 1, num_processes, *(observation_space[k]).shape
                )
            ...
            self.obs = torch.zeros(
                num_steps + 1, num_processes, *observation_space.shape
            )
    ...
    def insert(
        ...
        if self.is_lstm:
            self.recurrent_hidden_states[
                self.step + 1, :, : self.recurrent_hidden_state_size
            ].copy_(recurrent_hidden_states[0])
            self.recurrent_hidden_states[
                self.step + 1, :, self.recurrent_hidden_state_size :
            ].copy_(recurrent_hidden_states[1])
        else:
    ...
    def insert_truncated_obs(self, obs, index):
        if self.is_dict_obs:
            [
                self.truncated_obs[k][self.step + 1][index].copy_(to_tensor(obs[k]))
    ...
    def _compute_truncated_value_preds(self):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_processes):
                ...
                if self.is_dict_obs:
                    obs = {
                        k: self.truncated_obs[k][steps.squeeze(), i, :]
                        for k in self.truncated_obs.keys()
                    }
                else:
    ...
    def compute_gae_returns(self, returns_buffer, next_value, gamma, gae_lambda):
        ...
        if self.use_popart:
            self.denorm_value_preds = self.model.popart.denormalize(
                value_preds
            )  # denormalize all value predictions
    ...
    def compute_discounted_returns(self, returns_buffer, next_value, gamma):
        self.value_preds[-1] = next_value
        value_preds = self.value_preds
        ...
        for step in reversed(range(self.rewards.size(0))):
            returns_buffer[step] = (
                returns_buffer[step + 1] * gamma * self.masks[step + 1]
                + self.rewards[step]
            )
    ...
    def get_batched_value_loss(
        self, signed=False, positive_only=False, power=1, clipped=True, batched=True
    ):
        """
    ...
    def get_batched_action_complexity(self):
        ...
        for b in range(num_processes):
            ...
            for i, t in enumerate(done_steps[:-1]):
                ...
                action_str = " ".join(
                    [str(a.item()) for a in self.actions[t:next_done, b, 0]]
                )
    ...
    def feed_forward_generator(
        ...
        for indices in sampler:
            if self.is_dict_obs:
                obs_batch = {
                    k: self.obs[k][:-1].view(-1, *self.obs[k].size()[2:])[indices]
            ...
            if self.is_lstm:
                ...
                recurrent_hidden_states_batch = (
                    self._split_batched_lstm_recurrent_hidden_states(
                        recurrent_hidden_states_batch
                    )
                )
