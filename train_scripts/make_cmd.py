...
def generate_train_cmds(
    ...
    for t in range(num_trials):
        ...
        if xvfb:
            cmd = [
                f'xvfb-run -a -s "-screen 0 1400x900x24 +extension RANDR -noreset" -- python -m train'
            ]
        else:
...
def parse_args():
    ...
    parser.add_argument(
        "--use_ucb", action="store_true", help="Whether to include ucb arguments."
    )

    parser.add_argument("--xvfb", action="store_true", help="Whether to use xvfb.")
