...
def _make_env(args):
    ...
    if args.singleton_env:
        env_kwargs.update({"fixed_environment": True})
    ...
    if args.env_name.startswith("CarRacing"):
        ...
        if args.use_global_critic or args.use_global_policy:
            max_episode_steps = env._max_episode_steps
            env = TimeLimit(
                MultiGridFullyObsWrapper(env), max_episode_steps=max_episode_steps
            )
        return env
    else:
...
def create_parallel_env(args, adversary=True):
    ...
    venv = ParallelAdversarialVecEnv(
        [make_fn] * args.num_processes, adversary=adversary
    )
    ...
    venv = VecPreprocessImageWrapper(
        venv=venv, obs_key=obs_key, transpose_order=transpose_order, scale=scale
    )
