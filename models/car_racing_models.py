...
class CarRacingNetwork(DeviceAwareModule):
    def __init__(
        self, obs_shape, action_space, hidden_size=100, crop=False, use_popart=False
    ):
...
class CarRacingBezierAdversaryEnvNetwork(DeviceAwareModule):
    def __init__(
        ...
        self.base_output_size = (
            self.sketch_embed_dim + self.scalar_fc + self.random_z_dim
        )
    ...
    def _forward_base(self, inputs, rnn_hxs, masks):
        ...
        if self.use_goal:
            ...
            in_embedded = torch.cat(
                (in_sketch, in_time_step, in_z, in_goal_bin), dim=-1
            )
        else:
    ...
    def process_action(self, action):
        ...
        if self.use_categorical:
            x = ((action - 1.0) % self.sketch_dim[-1]) / self.sketch_dim[-1]
            y = ((action - 1.0) // self.sketch_dim[-2]) / self.sketch_dim[-2]
    ...
    def _sketch_to_mask(self, sketch):
        mask = torch.cat(
            (
                torch.zeros(sketch.shape[0], 1, dtype=torch.bool, device=self.device),
    ...
    def _is_goal_step(self, t):
        if self.use_goal:
            return (
                t == self.total_time_steps - 1
            )  # since time_step is +1 total time steps
        else:
            return False
    ...
    def act_random(self, inputs, rnn_hxs):
        ...
        if is_goal_step:
            ...
            action = torch.zeros(
                B, *action_shape, dtype=torch.int64, device=self.device
            )
            if self.use_categorical:
                action_high = (
    ...
    def evaluate_actions(self, inputs, rnn_hxs, masks, action):
        ...
        if self.use_goal:
            combined_log_probs = torch.zeros(
                (B, 1), dtype=torch.float, device=self.device
            )
            mean_entropy = 0
            if goal_steps.any():
