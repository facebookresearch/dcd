...
class Evaluator(object):
    ...
    def wrap_venv(venv, env_name, device="cpu"):
        is_multigrid = env_name.startswith("MultiGrid") or env_name.startswith(
            "MiniGrid"
        )
        is_car_racing = env_name.startswith("CarRacing")
...
def _get_poet_rose_env_names():
    env_names = [
        f"BipedalWalker-POET-Rose-{id}-v0"
    ...
    for xpid in xpids:
        ...
        if os.path.exists(checkpoint_path):
            ...
            dummy_venv = ParallelAdversarialVecEnv(
                make_fn, adversary=False, is_eval=True
            )
            ...
            # Load the agent
            agent = make_agent(
                name="agent", env=dummy_venv, args=xpid_flags, device=device
            )
